...
/*   Updated: 2018/04/13 23:36:04 by ynenakho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		*ft_getmode(t_flist *list)
{
	...
	*fmode++ = ft_getmodeacl((list->content.path) ? list->content.path :
...
static void	print_list_2(t_all *all, t_flist *list)
{
	if (all->flags.l)
	{
		...
		if (S_ISCHR(list->content.info.st_mode) ||
				S_ISBLK(list->content.info.st_mode))
...
void		print_list(t_all *all, t_flist *list)
{
	...
	while (cur)
	{
		print_list_2(all, cur);
		ft_printf("%s", cur->content.filename);
		if (S_ISLNK(cur->content.info.st_mode) && all->flags.l)
			ft_printf(" -> %s\n", get_linkpath(&cur->content));
		else
			ft_printf("\n");
		cur = cur->next;
		all->first++;
